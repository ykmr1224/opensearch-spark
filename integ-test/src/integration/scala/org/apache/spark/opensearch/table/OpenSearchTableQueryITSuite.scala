...
class OpenSearchTableQueryITSuite
  ...
  test("Query index with ip data type") {
    ...
    withIndexName(index1) {
      ...
      testQuery(s"SELECT client, server FROM $tableName",
          Seq(Row(IPAddress(clientIp(0)), IPAddress(serverIp)),
            Row(IPAddress(clientIp(1)), IPAddress(serverIp))))

      testQuery(s"SELECT client, server FROM $tableName WHERE client = string_to_ip('192.168.0.10')",
        Seq(Row(IPAddress(clientIp(0)), IPAddress(serverIp))))

      testQuery(s"SELECT client, server FROM $tableName WHERE ip_to_string(client) = '192.168.0.10'",
        Seq(Row(IPAddress(clientIp(0)), IPAddress(serverIp))))

      testQuery(s"SELECT client, server FROM $tableName WHERE ip_string_match(client, '192.168.0.10')",
        Seq(Row(IPAddress(clientIp(0)), IPAddress(serverIp))))

      testQuery(s"SELECT client, server FROM $tableName WHERE string_ip_match('192.168.0.10', client)",
        Seq(Row(IPAddress(clientIp(0)), IPAddress(serverIp))))

      // Equality check is done by string equality (limitation of Spark UDT), and it won't match
      testQuery(s"SELECT client, server FROM $tableName WHERE client = string_to_ip('::ffff:192.168.0.10')",
        Seq())

      // Need to use ip_string_match/string_ip_match to match different notation
      testQuery(s"SELECT client, server FROM $tableName WHERE ip_string_match(client, '::ffff:192.168.0.10')",
        Seq(Row(IPAddress(clientIp(0)), IPAddress(serverIp))))
    }
  }
